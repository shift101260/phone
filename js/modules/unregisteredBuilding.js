...
window.addUnregBuildingRow = function(data = {}) {
    ...
    const html = `
        <div id="${rowId}" class="grid grid-cols-[1.2fr_1fr_1fr_1.2fr_32px] gap-2 bg-white p-2 rounded-lg border border-slate-200 shadow-sm items-center text-xs">
            <input type="text" value="${data.address || ''}" placeholder="例：廠房後方鐵皮增建" class="w-full px-2 py-1 border border-slate-300 rounded-lg">
            <input type="text" value="${data.structure || ''}" placeholder="例：鋼架造" class="w-full px-2 py-1 border border-slate-300 rounded-lg">
            <input type="text" value="${data.area || ''}" placeholder="例：80坪" class="w-full px-2 py-1 border border-slate-300 rounded-lg font-mono">
            <input type="text" value="${data.note || ''}" placeholder="例：倉儲使用 / 可辦理補照" class="w-full px-2 py-1 border border-slate-300 rounded-lg">
